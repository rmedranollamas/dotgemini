...
try:
    ...
    print('Warning: pyyaml is not installed. Skipping validation.')
    sys.exit(0)
...
" 2>&1)
    ...
    if [ $EXIT_CODE -ne 0 ]; then
        STATUS="FAILED"
        REASON=$(printf "SYNTAX ERROR: I can't parse %s. Fix the broken YAML structure before retrying: %s" "$FILE_PATH" "$OUTPUT")
        jq -n --arg r "$REASON" \
            '{
                decision: "deny",
                reason: $r
