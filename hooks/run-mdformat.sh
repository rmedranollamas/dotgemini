...
if [[ "$FILE_PATH" == *.md ]]; then
    ...
    if [ $EXIT_CODE -ne 0 ]; then
        STATUS="FAILED"
        REASON=$(printf "FORMATTING ERROR: I can't format %s. Check for broken Markdown syntax: %s" "$FILE_PATH" "$FORMAT_OUT")
        jq -n --arg r "$REASON" \
            '{
                decision: "deny",
                reason: $r
        ...
        AGENT_MSG=$(printf "VERIFIED: I've auto-formatted %s to match project standards. Carry on." "$FILE_PATH")
